...
self.addEventListener('notificationclick', function(event) {
  ...
  // ✅ IMPORTANT: event.notification.data contains the data from the push payload
  const data = event.notification.data || {};
  const action = event.action || '';
  
  console.log('[Service Worker] Action:', action);
  console.log('[Service Worker] Data from notification:', JSON.stringify(data));
  ...
  if (action === 'reply') {
    if (data.chatId) {
      ...
      console.log('[Service Worker] ❌ No chatId or orderId found in data!');
    ...
    console.log('[Service Worker] Default click →:', targetUrl);
  }
...
messaging.onBackgroundMessage((payload) => {
  ...
  // ✅ Get data from payload.data (this is what your Cloud Function sends)
  const notification = payload.notification || {};
  const data = payload.data || {};
  
  console.log('[Service Worker] Received data:', JSON.stringify(data));
  console.log('[Service Worker] chatId from data:', data.chatId);
  console.log('[Service Worker] orderId from data:', data.orderId);
  
  const notificationTitle = notification.title || data.title || 'New Update!';
  
  // ✅ Pass the data to the notification so it's available on click
  ...
  console.log('[Service Worker] Notification data being set:', JSON.stringify(notificationOptions.data));
